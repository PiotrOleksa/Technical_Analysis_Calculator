...
class TechnicalIndicatorsChartPlotter:
    def plot_price_and_signals(self, fig, crypto, data, strategy, axs):
        ...
        fig.suptitle(f'Top: {crypto.symbol} Stock Price. Bottom: {strategy}')
        ...
        axs[0].plot(crypto.prices, label='Close Price', color='blue', alpha=0.35)
    ...
    def plot_macd(self, crypto):
        image = f'images/{crypto.symbol}_macd.png'
        macd = crypto.technical_indicators

        
        fig, axs = plt.subplots(2, sharex=True, figsize=(13,9))
        self.plot_price_and_signals(fig, crypto, macd, 'MACD', axs)

        axs[1].plot(macd['MACD'], label=crypto.symbol+' MACD', color = 'green')
    ...
    def plot_rsi(self, crypto):
        image = f'images/{crypto.symbol}_rsi.png'
        rsi = crypto.technical_indicators
        low_rsi = 40
        high_rsi = 70

        
        fig, axs = plt.subplots(2, sharex=True, figsize=(13, 9))
        self.plot_price_and_signals(fig, crypto, rsi, 'RSI', axs)
    ...
    def plot_bollinger_bands(self, crypto):
        image = f'images/{crypto.symbol}_bb.png'
        bollinger_bands = crypto.technical_indicators

        fig, axs = plt.subplots(2, sharex=True, figsize=(13, 9))

        self.plot_price_and_signals(fig, crypto, bollinger_bands, 'Bollinger_Bands', axs)
